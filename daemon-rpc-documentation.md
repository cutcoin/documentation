...
This is a list of the cutcoind daemon RPC calls, their inputs and outputs, and examples of each.
...
*   [get_pos_block_template](#get_pos_block_template)
*   [submit_block](#submit_block)
*   [submit_pos_block](#submit_pos_block)
...
The majority of cutcoind RPC calls use the daemon's `json_rpc` interface to request various bits of information. These methods all follow a similar structure, for example:
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_block_count"}' -H 'Content-Type: application/json'  
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"on_get_block_hash","params":[912345]}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_block_template","params":{"wallet_address":"cutb9vuXgqtS2fDGRRptm8QHdQb4NQv1ZWBDhUCzQAjYcCggvgTcvSsZX6WetiXw1p3HpTgAhTdmniSRwzqKNPDW4mUQt5FZsW","reserve_size":60}' -H 'Content-Type: application/json'
...
### **get_pos_block_template**

Get a block template on which mining a new block.

Alias: _getposblocktemplate_.

Inputs:

*   _wallet_address_ - string; Address of wallet to receive coinbase transactions if block is successfully minted.
*   _pos_tx_size_ - unsigned int; PoS tx size; max 255 bytes.
*   _timestamp_ - unsigned int; Timestamp.

Outputs:

*   _blocktemplate_blob_ - string; Blob on which to try to mine a new block.
*   _merkleroot_ - string; Merkle root.
*   _difficulty_ - unsigned int; Difficulty of next block.
*   _expected_reward_ - unsigned int; Coinbase reward expected to be received if block is successfully mined.
*   _height_ - unsigned int; Height on which to mine.
*   _prev_hash_ - string; Hash of the most recent block on which to mine the next block.
*   _prev_crypto_hash_ - string; Previous crypto hash.
*   _status_ - string; General RPC error code. "OK" means everything looks good.
*   _untrusted_ - boolean; States if the result is obtained using the bootstrap mode, and is therefore not trusted (`true`), or when the daemon is fully synced (`false`).

Example:

    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_pos_block_template","params":{"wallet_address":"cutb9vuXgqtS2fDGRRptm8QHdQb4NQv1ZWBDhUCzQAjYcCggvgTcvSsZX6WetiXw1p3HpTgAhTdmniSRwzqKNPDW4mUQt5FZsW","pos_tx_size":60,"timestamp":1615505870}' -H 'Content-Type: application/json'
...
Alias: _submitposblock_.

Inputs:

*   _block_blob_ - string; block blob data. See [get_pos_block_template](#get_pos_block_template) to get a block blob.
*   _pos_tx_blob_ - string; coinstake tx blob data.
...
Alias: _generateblocks_.

Inputs:

*   _wallet_address_ - string; Address of wallet to receive coinbase transactions if block is successfully minted.
*   _amount_of_blocks_ - unsigned int; PoS tx size; max 255 bytes.
*   _timestamp_ - unsigned int; Timestamp.
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"generateblocks","params":{"wallet_address":"cutb9vuXgqtS2fDGRRptm8QHdQb4NQv1ZWBDhUCzQAjYcCggvgTcvSsZX6WetiXw1p3HpTgAhTdmniSRwzqKNPDW4mUQt5FZsW","amount_of_blocks":1000000}}' -H 'Content-Type: application/json'
...
*   _block_header_ - A structure containing block header information.
    ...
    *   _difficulty_ - unsigned int; The strength of the Cutcoin network based on mining power.
    *   _hash_ - string; The hash of this block.
    *   _height_ - unsigned int; The number of blocks preceding this block on the blockchain.
    *   _major_version_ - unsigned int; The major version of the cutcoin protocol at this block height.
    *   _minor_version_ - unsigned int; The minor version of the cutcoin protocol at this block height.
    *   _nonce_ - unsigned int; a cryptographic random one-time number used in mining a Cutcoin block.
    ...
    *   _reward_ - unsigned int; The amount of new atomic units generated in this block and rewarded to the miner. Note: 1 CUT = 1e12 atomic units.
...
In this example, the most recent block (1562023 at the time) is returned:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_last_block_header"}' -H 'Content-Type: application/json'
...
In this example, block 912345 is looked up by its hash:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_block_header_by_hash","params":{"hash":"e22cf75f39ae720e8b71b3d120a5ac03f0db50bba6379e2850975b4859190bc6"}}' -H 'Content-Type: application/json'
...
In this example, block 912345 is looked up by its height (notice that the returned information is the same as in the previous example):
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_block_header_by_height","params":{"height":912345}}' -H 'Content-Type: application/json'
...
*   _block_mining_info_ - unsigned int; Cumulative difficulty.
    *   _difficulty_ - string; Current difficulty.
    *   _height_ - unsigned int; Current height.
    *   _pos_hash_ - string; Last PoS hash.
    *   _timestamp_ - unsigned int; Timestamp.

Example:

    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_mining_info"}' -H 'Content-Type: application/json'
...
In this example, blocks range from height 1545999 to 1546000 is looked up (notice that the returned informations are ascending order and that it is at the April 2018 network upgrade time):
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_block_headers_range","params":{"start_height":1545999,"end_height":1546000}}' -H 'Content-Type: application/json'
...
In the following example, block 912345 is looked up by its height. Note that block 912345 does not have any non-coinbase transactions. (See the next example for a block with extra transactions):
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_block","params":{"height":912345}}' -H 'Content-Type: application/json'
...
In the following example, block 993056 is looked up by its hash. Note that block 993056 has 3 non-coinbase transactions:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_block","params":{"hash":"510ee3c4e14330a7b96e883c323a60ebd1b5556ac1262d0bc03c24a3b785516f"}}' -H 'Content-Type: application/json'
...
Following is an example of `get_connections` and it's return:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_connections"}' -H 'Content-Type: application/json'
...
*   _bootstrap_daemon_address_ - string; [Bootstrap node](/resources/cutcoinpedia/bootstrap-node.html) to give immediate usability to wallets while syncing by proxying RPC to it. (Note: the replies may be untrustworthy).
...
Following is an example `get_info` call and its return:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_info"}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"hard_fork_info"}' -H 'Content-Type: application/json'
...
In the following example, host is banned with its IP address string-formatted as A.B.C.D:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"set_bans","params":{"bans":[{"host":"192.168.1.51","ban":true,"seconds":30}]}}' -H  'Content-Type: application/json'
...
In the following example, integer-formatted IP is banned:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"set_bans","params":{"bans":[{"ip":838969536,"ban":true,"seconds":30}]}}' -H  'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_bans"}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"flush_txpool","params":{"txids":["dc16fa8eaffe1484ca9014ea050e13131d3acf23b419f33bb4cc0b32b6c49308",""]}}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_output_histogram","params":{"amounts":[20000000000]}}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_coinbase_tx_sum","params":{"height":1563078,"count":2}}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_version"}' -H 'Content-Type: application/json'
...
*   _fee_ - unsigned int; Amount of fees estimated per byte in atomic units
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_fee_estimate"}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_alternate_chains"}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"relay_tx","params":{"txids":["9fd75c429cbe52da9a52f2ffc5fbd107fe7fd2099c0d8de274dc8a67e0c98613"]}}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"sync_info"}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_txpool_backlog"}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/json_rpc -d '{"jsonrpc":"2.0","id":"0","method":"get_output_distribution","params":{"amounts":[628780000],"from_height":1462078}}' -H 'Content-Type: application/json'
...
For calls that end with **.bin**, the data is exchanged in the form of binary, serialized objects, as defined in the [Core RPC Server](https://github.com/cutcoin-project/cutcoin/blob/master/src/rpc/core_rpc_server_commands_defs.h).
...
*   _untrusted_ - boolean; States if the result is obtained using the bootstrap mode, and is therefore not trusted (`true`), or when the daemon is fully synced (`false`).
    ...
    $ curl http://127.0.0.1:24248/get_height -H 'Content-Type: application/json'
...
Return list of all tokens present in blockchain. Binary request.
...
Example 1: Return transaction information in binary format.
    ...
    $ curl http://127.0.0.1:24248/get_transactions -d '{"txs_hashes":["d6e48158472848e6687173a91ae6eebfa3e1d778e65252ee99d7515d63090408"]}' -H 'Content-Type: application/json'
...
Example 2: Decode returned transaction information in JSON format. Note: the "vin", "vout" and "signatures" list have been truncated in the displayed return for space considerations.
    ...
    $ curl http://127.0.0.1:24248/get_transactions -d '{"txs_hashes":["d6e48158472848e6687173a91ae6eebfa3e1d778e65252ee99d7515d63090408"],"decode_as_json":true}' -H 'Content-Type: application/json'
...
Example 3: Returned a missed (unexisting) transaction.
    ...
    curl http://127.0.0.1:24248/get_transactions -d '{"txs_hashes":["d6e48158472848e6687173a91ae6eebfa3e1d778e65252ee99d7515d63090409"]}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/get_alt_blocks_hashes -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/is_key_image_spent -d '{"key_images":["8d1bd8181bf7d857bdb281e0153d84cd55a3fcaa57c3e570f4a49f935850b5e3","7319134bfc50668251f5b899c66b005805ee255c136f0e1cecbb0f3a912e09d4"]}' -H 'Content-Type: application/json'
...
Example (No return information included here.):
    ...
    $ curl http://127.0.0.1:24248/send_raw_transaction -d '{"tx_as_hex":"de6a3...", "do_not_relay":false}' -H 'Content-Type: application/json'
...
*   _block_time_ - unsigned int; Target block time.
*   _algorithm_ - string; Algorithm version.
*   _pos_hash_ - string; PoS hash.
*   _height_ - unsigned int; Current height.
*   _status_ - string; General RPC error code. "OK" means everything looks good. Any other value means that something went wrong.
*   _difficulty_ - unsigned int; Cumulative difficulty.

Example:

    $ curl http://127.0.0.1:24248/staking_status -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/save_bc -H 'Content-Type: application/json'
...
*   _gray_list_ - array of offline _peer_ structure as follows:
    ...
    *   _port_ - unsigned int; TCP port the peer is using to connect to cutcoin network.
*   _status_ - string; General RPC error code. "OK" means everything looks good. Any other value means that something went wrong.
*   _white_list_ - array of online _peer_ structure, as above.

Example (truncated lists):

    $ curl http://127.0.0.1:24248/get_peer_list -H 'Content-Type: application/json'
...
Example while mining:
    ...
    $ curl http://127.0.0.1:24248/set_log_hash_rate -d '{"visible":true}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/set_log_level -d '{"level":1}' -H 'Content-Type: application/json'
...
Example to set all facilities to Security Level `Info`:
    ...
    $ curl http://127.0.0.1:24248/set_log_categories -d '{"categories": "*:INFO"}' -H 'Content-Type: application/json'
...
Example without input to set the default categories:
    ...
    $ curl http://127.0.0.1:24248/set_log_categories -H 'Content-Type: application/json'
...
Example (Note: Some lists in the returned information have been truncated for display reasons):
    ...
    $ curl http://127.0.0.1:24248/get_transaction_pool -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/get_transaction_pool_hashes.bin -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/get_transaction_pool_stats -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/stop_daemon -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/get_limit -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/set_limit -d '{"limit_down": 1024}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/out_peers -d '{"out_peers": 3232235535}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/out_peers -d '{"in_peers": 3232235535}' -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/start_save_graph -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/stop_save_graph -H 'Content-Type: application/json'
...
Example:
    ...
    $ curl http://127.0.0.1:24248/update -d '{"command":"check"}' -H 'Content-Type: application/json'
